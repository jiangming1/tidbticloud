...
就是帮助tidb在企业中推广应用。过去数据库是一个神秘的东西。公司会觉得他很重要。会付给dba很多薪资。生活中的确也是很重要的，一个核心数据库的宕机会让企业整个公司业务无法开展。原先我职业生涯中遇到一个数据库问题。机房宕机了。导致整个中国必胜客收银都无法使用。当天必胜客就损失2000w。最近元神和必胜客的联名活动也是这样。整个付款业务无法使用。这对企业造成了巨大的损失。
我们引入k8s,tidb希望真正能解决这个问题。
在云的时代。平时用小数据库。遇到流量爆发的时候。用几百台物理机组成的大数据库。这就是我们平台的目的。而tidb数据库如果支撑大并发需要很多物理机。很多硬件资源处在闲置浪费状态。我们在k8s上引入tidb就是cfo最喜欢的省钱功能。
